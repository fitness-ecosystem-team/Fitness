...
type UserContext = {
  user: { name: string };
  subscription: { plan: string; canAccessFeatures: boolean };
  goals: any[];
  activityToday: any;
...
export default function HomeScreen() {
  ...
  const isPro = ctx?.subscription.canAccessFeatures;
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsRow}>
          <StatCard label="Steps" value={ctx?.activityToday?.steps ?? 0} color={colors.info} />
          <StatCard label="Calories" value={ctx?.activityToday?.caloriesBurned ?? 0} unit="kcal" color={colors.warning} />
          <StatCard label="Active min" value={ctx?.activityToday?.activeMinutes ?? 0} color={colors.success} />
        </View>
        ...
        {(ctx?.unreadNotifications ?? 0) > 0 && (
          <View style={styles.notifBanner}>
            <Text style={styles.notifText}>
              🔔 You have {ctx?.unreadNotifications} unread notification{ctx!.unreadNotifications > 1 ? 's' : ''}
            </Text>
          </View>
        )}
      </ScrollView>
